...
Vote.methods.setPoints = function() {
  ...
  var bonusPoints = 0;
  return Promise.all([
    this.model('Period').findById(this.period),
    this.model('Candidate').findById(this.candidate)
  ])
  .then((values) => {
...
Vote.pre('validate', function(next) {
  ...
  .then((votes) => {
    ...
    if (votes.length > (vote ? 1 : 0)) {
      ...
      return this.model('Period').findById(this.period);
    }
  })
  .then((period) => {
    if (moment().isAfter(period.end)) {
      ...
      return this.model('Candidate').findById(this.candidate);
    }
  })
  .then((candidate) => {
